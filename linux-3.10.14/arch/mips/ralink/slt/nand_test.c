...
int _nfc_read_raw_data(int cmd1, int cmd2, int bus_addr, int bus_addr2, int conf, char *buf, int len, int flags)
{
	int ret;
...
EXPORT_SYMBOL(_nfc_read_raw_data);
...
int nfc_read_page(struct ra_nand_chip *ra, char *buf, int page, int flags)
{
	...
	while (size > 0) {
		...
		if (status & NAND_STATUS_FAIL) {
			printk("%s: fail page=%x, addr=%08x, addr2=%08x\n", __func__, page, bus_addr, bus_addr2);
			return -EIO;
		}
...
int ra_nand_init(void) 
{
	...
	int alloc_size, bbt_size, buffers_size, chip_mode, reg;
	
	tmp = ra_inl(RALINK_SYSCTL_BASE + 0x860);
	tmp |= (1L << 8);
	ra_outl(RALINK_SYSCTL_BASE + 0x860, tmp);
...
#define ALIGNE_16(a) (((unsigned long)(a)+15) & ~15)
	...
	//printk("%s: alloc %x, at %p , btt(%p, %x)\n", 
	//       __func__ , alloc_size, ra, ra->bbt, bbt_size);
...
int test_nand(void)
{
	ranfc_verify = 1;
	nfc_wait_ready(0);
	ramtd_nand_erase(NULL, NULL);
	ramtd_nand_write(NULL, 0, 0, 0, NULL);
	ramtd_nand_read(NULL, 0, 0, 0, NULL);
	ramtd_nand_readoob(NULL, 0, NULL);
	ramtd_nand_writeoob(NULL, 0, NULL);
	ramtd_nand_block_isbad(NULL, 0);
	ramtd_nand_block_markbad(NULL, 0);

	return 0;
}
#endif /* CONFIG_RALINK_RT6855A */

int slt_nand_test(unsigned int chip_id)
{
#if defined (CONFIG_RALINK_RT6855A)
	unsigned char target_nand_id[][4] = {
		{0xc2, 0xf1, 0x80, 0x1d},
		{0x2c, 0xdc, 0x90, 0xa6},
		{0x92, 0xf1, 0x80, 0x95}};					
	...
	if ((chip_id == RT6855A_CHIP)) {
		...
		ra_nand_init();
		mdelay(1000);
		_nfc_read_raw_data(0x90, 0, 0, 0, 0x410141, id, 4, 2);
		printk("SLT NAND Test: NAND flash id: %x %x %x %x\n", id[0], id[1], id[2], id[3]);
		for (i = 0; i < sizeof(target_nand_id) / sizeof(id); i++) {
			if (memcmp(target_nand_id[i], id, sizeof(id)) == 0)
				return 1;
		}
		return 0;
	}
