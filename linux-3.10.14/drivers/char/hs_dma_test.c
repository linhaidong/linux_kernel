...
#define PATTERN_BOUNDARY 33000 //for unalignment memcpy test
...
unsigned char *Src[NUM_HSDMA_TX_DESC];
unsigned char *Dst[NUM_HSDMA_TX_DESC];

dma_addr_t Src_phy[NUM_HSDMA_TX_DESC];
dma_addr_t Dst_phy[NUM_HSDMA_TX_DESC];

unsigned char DoneBit[NUM_HSDMA_TX_DESC];
	...
	if (mode==0){	
		...
		while(1){
			for(i=8;i<16384;i++){
				for (times=0; times < (NUM_HSDMA_TX_DESC); times++ ){
				//printk("times=%d \n", times);
					HS_DmaMem2MemGenUse(Src_phy[times], Dst_phy[times], i, Done);
				}
			}	
		}  
...
static int RalinkGdmaTestInit(void)
{
	...
	//alloc phy memory for src
		Src[0] = pci_alloc_consistent(NULL, PATTERN_BOUNDARY, &Src_phy[0]);
	  if(Src[0]==NULL) {
	...
	//alloc phy memory for dst
	  Dst[0] = pci_alloc_consistent(NULL, PATTERN_BOUNDARY, &Dst_phy[0]);
		if(Dst[0]==NULL) {
